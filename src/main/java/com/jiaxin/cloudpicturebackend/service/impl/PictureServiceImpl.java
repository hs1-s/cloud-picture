...
import com.jiaxin.cloudpicturebackend.common.CaffeineService;
...
import com.jiaxin.cloudpicturebackend.manager.upload.FilePictureUpload;
import com.jiaxin.cloudpicturebackend.manager.upload.PictureUploadTemplate;
import com.jiaxin.cloudpicturebackend.manager.upload.UrlPictureUpload;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    /**
     * 上传图片
     * @param inputSource   由此参数判断是根据文件上传还是 url上传
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null){
            ...
            // 更新图片信息
            newPicture = setPictureFields(uploadPictureResult, loginUser, spaceId);
            // 新增需要额外插入图片id和编辑时间
     ...
     * @return
     */
    public Picture setPictureFields(UploadPictureResult uploadPictureResult, User loginUser, Long spaceId){
    ...
    public Page<PictureVO> listPictureVOByPageWithcache(PictureQueryRequest pictureQueryRequest, HttpServletRequest request) {
        ...
        // 2.构建缓存 key
        String queryCondition = JSONUtil.toJsonStr(pictureQueryRequest); //将查询条件转换为 json字符串
        String hashKey = DigestUtils.md5DigestAsHex(queryCondition.getBytes()); //使用 MD5 算法生成对应的哈希值，对key长度进行压缩
        String cacheKey = "cloudpicture:listPictureVOByPage:" + hashKey;

        // 3.先查询本地缓存 (Caffeine)
        String cacheValue = caffeineService.getIfPresent(cacheKey);
        if (cacheValue != null && cacheValue.length() != 0){
            // 将缓存中的 JSON 字符串转换为 Page 类型的对象。
        ...
        if (cacheValue != null && cacheValue.length() != 0){
            //如果命中，先存入本地缓存
            caffeineService.put(cacheKey, cacheValue);
            // 查 Redis，返回结果
            Page<PictureVO> pictureVOPage = JSONUtil.toBean(cacheValue, Page.class);
            return pictureVOPage;
        }
        // 5.都没有，查数据库，并将数据缓存到两个缓存中去
        ...
        int timeOut = 5 + rand.nextInt(11);
        opsForValue.set(cacheKey, cacheValue, timeOut, TimeUnit.MINUTES);
        // 6.获取封装类
        return pictureVOPage;
    }
